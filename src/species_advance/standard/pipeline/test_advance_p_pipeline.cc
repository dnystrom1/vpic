                                ...
                                int n_pipeline )
{
  ...
  for( int i = 0 ; i < n; i++, ipart++ )
  {
    ...
    ux_old = ux + wdn_zero;                   // Old momentum + fake
    uy_old = uy + wdn_zero;
    uz_old = uz + wdn_zero;
    ...
    pb[ib].ux[ip] = ux_old;                   // Store momentum
    pb[ib].uy[ip] = uy_old;
    pb[ib].uz[ip] = uz_old;
         ...
         -v3 <= one && -v4 <= one && -v5 <= one )
    {
      ...
      pb[ib].dx[ip] = dx;                     // Store new position
      pb[ib].dy[ip] = dy;
      pb[ib].dz[ip] = dz;

      // pb[ib].dx[ip] = v3;                     // Store new position
      // pb[ib].dy[ip] = v4;
      // pb[ib].dz[ip] = v5;
