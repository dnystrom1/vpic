...
// Here, we make a first implementation of a v4 version.
int
move_p( particle_block_t * RESTRICT ALIGNED(128) pb,
        ...
        const float                              qsp )
{
  ...
  r[1] = pb[ib].dy[ip];
  r[2] = pb[ib].dz[ip];
  // r[3] = pb[ib].i [ip]; // Do not need this. Experiment with not loading.
  ...
  q3 = v4float( 1.0f / 3.0f ) * q;     // q3 = p_q/3, p_q/3, p_q/3, D/C
  ...
  for(;;)
  {
    ...
    //
    //   sgn_dr indicates the sign of streak displacements.  This is
    ...
    // local voxel should happen most of the time once we get to
